...
[*Reference*](https://www.dell.com/support/manuals/en-us/dell-emc-os-9/s5048f-on-9.14.2.4-config/using-rsa-authentication-of-ssh?guid=guid-c6580e5b-9cf4-4791-aa5f-eefb3188109b&lang=en-us)
...
2. The `vlt-peer-lag` command associates `port-channel 10` of this VLT peer to the corresponding `port-channel 10` of the other VLT peer for the VLT LAG connection to an attached device.
...
!!! note
    ...
    Don't forget to create a LAG with LACP on the attached device and configure LACP as well.
