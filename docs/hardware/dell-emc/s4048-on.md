...
!!! info
    ...
    When a port is in `switchport` mode, it passes mlutiple VLANs, or one untagged VLAN. It cannot accept both untagged and tagged traffic.
...
!!! info
    ...
    In hybrid `switchport` mode, the interface may pass bothj untagged and tagged VLANs.
