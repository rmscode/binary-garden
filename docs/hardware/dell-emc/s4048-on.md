...
To receive periodic power updates, you must enable the following command: `enable optic-info-update interval`. When the syste detects a genuine over-temperature condition, it power off the line card. To recognize this condition, look for the following system messages:
