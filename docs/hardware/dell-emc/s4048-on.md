...
To display the logging buffer, use the `show logging` command.
...
ToDo . . . The steps to configure a UNIX system as a syslog server needs to be documented.

Ubuntu is probably the easiest to set up. The line `local7.debugging /var/log/ftos.log` can be added to `/etc/syslog.conf `.
