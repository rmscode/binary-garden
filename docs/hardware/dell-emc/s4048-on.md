...
When you access the CLI on a fresh system, you will be prompted to activate Support Assist. Enter CONFIGURE mode to make changes to the support assist package. Taking the prompt at face value and entering `support-assist activate` in ENABLE mode will fail on the word `activate`.
...
To manually set the time, use the `clock set <time> <month> <day> <year>` command:

```shell
DellEMC#clock set 16:20:00 9 september 2017
```

!!! note

    `clock set` did not work when we tested for whatever reason despite that being whats listed in the Dell OS9 (9.14) docs. We need to find the correct command...maybe we have a different version.
...
=== "VLT Peer 1"
    ...
    4. (Optional) The system elects a primary and secondary VLT device, but you can confugure the roles before the election process by setting the `primary-priority` (1-65535). The lowest value is elected as the primary.
